...
class FirebaseRepository {
    ...
    fun listenToProgress(
        ...
        .addSnapshotListener { snapshot, error ->
            ...
            if (snapshot != null && snapshot.exists()) onChange(snapshot.toProgress())
            else onChange(Progress())
        }

    fun listenToGoals(
        uid: String,
        onChange: (List<StudyGoal>) -> Unit,
        onError: (Throwable) -> Unit = {}
    ): ListenerRegistration = db.collection("users").document(uid)
        .collection("goals")
    ...
    fun listenToHomework(
        uid: String,
        onChange: (List<HomeworkItem>) -> Unit,
        onError: (Throwable) -> Unit = {}
    ): ListenerRegistration = db.collection("users").document(uid)
        .collection("homework")
        .orderBy("createdAt", Query.Direction.DESCENDING)
    ...
    fun completeGoal(goalId: String, onResult: (Result<Unit>) -> Unit = {}) {
        completeItem("goals", goalId, "goalsCompleted", onResult)
    }

    fun addHomework(subject: String, title: String, onResult: (Result<Unit>) -> Unit = {}) {
        val uid = auth.currentUser?.uid ?: return onResult(Result.failure(IllegalStateException("Not signed in")))
        if (subject.isBlank() || title.isBlank()) {
            return onResult(Result.failure(IllegalArgumentException("Subject and task are required")))
        }
    ...
    fun completeHomework(itemId: String, onResult: (Result<Unit>) -> Unit = {}) {
        completeItem("homework", itemId, "homeworkCompleted", onResult)
    }
    ...
    fun recordQuizResult(correct: Int, total: Int, onResult: (Result<Unit>) -> Unit = {}) {
        val uid = auth.currentUser?.uid ?: return onResult(Result.failure(IllegalStateException("Not signed in")))
        if (total <= 0 || correct !in 0..total) {
            return onResult(Result.failure(IllegalArgumentException("Invalid quiz result")))
        }
        val ref = db.collection("users").document(uid).collection("progress").document("current")
        ref.set(
            mapOf(
